...
class BotHandler(object):
    ...
    def __init__(self, token, dialogue_manager):
        ...
        self.dialogue_manager = dialogue_manager   #引入了对话管理器类

    def get_updates(self, offset=None, timeout=30):
        params = {"timeout": timeout, "offset": offset}
        raw_resp = requests.get(urljoin(self.api_url, "getUpdates"), params)
        #print("raw_resp.json()", raw_resp.json())
        #print(urljoin(self.api_url, "getUpdates"))
        try:
            resp = raw_resp.json()
        ...
        if "result" not in resp:
            #print("resp:", resp)
            #print("sorry, result not in resp")
            return []
        return resp["result"]
    ...
    def get_answer(self, question):
        ...
        return self.dialogue_manager.generate_answer(question)     #使用generate_answer方法进行回复
...
def main():
    ...
    #bot = BotHandler(token, simple_manager)    #现在采用的SimpleDialogueManager,后续必须改进成自己的DialogueManager
            ...
            if "message" in update:
                ...
                if "text" in update["message"]:
                    ...
                    if is_unicode(text):
                        ...
                        #这里的update["messge"]["text"]表示的应该就是question
                        #发送信息．get_answer调用generate_answer
